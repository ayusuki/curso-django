...
def contato_simples(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            contato = Contato(nome=form.cleaned_data['nome'],
...
def contato_fones(request):
    ...
    if request.method == 'POST':
        ...
        if form_contato.is_valid() and forms_fones.is_valid():
            contato = Contato(nome=form_contato.cleaned_data['nome'],
            ...
            for form_fone in forms_fones:
                ...
                fone.save()
            return render(request, 'contatos/contato_ok.html', {'form_contato':form_contato,
                                                                'forms_fones': forms_fones,
                                                                })
    else:
        form_contato = ContatoSimples()
        forms_fones = FonesFormSet()
    return render(request, 'contatos/contato_fones.html', {'form_contato': form_contato,
                                                            'forms_fones': forms_fones,
                                                          })                                                          
